...
- **Faster testing** (e.g., by utilizing [test impact analysis](https://martinfowler.com/articles/rise-test-impact-analysis.html)/test selection and prioritization) and **more testing** (e.g., running a large number of simulation-based end-to-end tests on the cloud).
- **Faster build time** (e.g., via [remote caching](https://bazel.build/remote/caching) using [Bazel](https://discourse.ros.org/t/adding-bazel-as-an-official-alternative-build-system-for-ros2-iron/28476/19)), and **scalable merge workflow** (e.g., using MergeQueue).
- **Adopting programmatic orchestration** (e.g., Kubernetes Federation) in preparation for large-scale, cross-team agility to support large-scale architectural and infrastructure changes.
...
I thank all my colleagues who have engaged in conversations with me on related topics.
