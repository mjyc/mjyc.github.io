...
- Setting up **continuous integration or nightly tests** (e.g., using Jenkins, GitLab CI/CD, GitHub Actions) with **end-to-end tests involving a high-fidelity simulator** (e.g., Gazebo) to quickly smoke test rapidly changing codebases.
...
- Establishing a **deployment strategy with rollback support** (e.g., using tools that enable infrastructure-as-code/gitops like Ansible, Docker, and Jenkins or Terraform and Kubernetes/k3s) to avoid manually applying untrackable hotfixes in fear of losing customers.
...
- Implementing **nightly tests on real robots** in a mirror warehouse or manufacturing line or **[physical continuous integration](https://youtu.be/JNV9CkARh**g) in production-like environments** to prevent edge and corner cases and performance regressions in production environments as much as possible.
...
- **Faster testing** (e.g., via [TIA](https://martinfowler.com/articles/rise-test-impact-analysis.html)/test selection and prioritization) and **more testing** (e.g., running a large number of simulation-based end-to-end tests on the cloud).
- **Faster build time** (e.g., via [remote caching](https://bazel.build/remote/caching)), **build promotion**, and **scalable merge workflow** (e.g., using MergeQueue).
- **Adopting programmatic orchestration** in preparation for large-scale (e.g., Kubernetes), cross-team agility to support large-scale architectural and infrastructure changes.
- **Hardware-in-the-loop (HIL) testing** (e.g., using Gazebo, Isaac Sim).
- **Enhancing data record and replayer** to support streaming robotics data (e.g., instead of waiting for downloading), jumping between critical/salient timestamps, and better integration with high-fidelity simulators (e.g., handling high throughput).
- **More simulation tooling** (e.g., test scenario generator).

### Beyond 2000 Employees

Typically, companies at this stage are not considered startups, but since robotics is an experimental technology, robotics companies are still considered startups at this stage.
However, I don't have any firsthand experience with companies at this size, and in fact, I only know a handful of robotics companies at this stage, such as large autonomous vehicle companies like Waymo, Cruise, Aurora, Zoox.
One interesting trend I've seen is custom hardware to accelerate computations unique to their needs, but that's just speculation.
