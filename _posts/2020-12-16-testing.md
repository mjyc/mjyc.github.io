...
- Faster testing, e.g., via [TIA](https://martinfowler.com/articles/rise-test-impact-analysis.html)/test selection and prioritization
- More (simulation) testing, e.g., on the cloud
- Faster build time, e.g., via [remote caching](https://bazel.build/remote/caching) 
- Scale merge workflow
- Adopting programmatic orchestration in preparation for large-scale, cross-team agility, e.g., to support large-scale architectural and infrastructure changes.
- Hardware-in-the-loop (HIL) testing
- Enhancing data record and replayer to support streaming robotics data (e.g., instead of waiting for downloading), jumping between critical/salient timestamps, better integration with high-fidelity simulators (e.g., handling high throughput).
- More simulation tooling
